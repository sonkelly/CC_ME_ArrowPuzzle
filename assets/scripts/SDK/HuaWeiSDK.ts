...
export class HuaWeiSDK extends AbstractPlatformSDK {
    private static _instance: HuaWeiSDK;
    ...
    static getInstance(): HuaWeiSDK {
        if (this._instance === undefined) {
            this._instance = new HuaWeiSDK();
        }
    ...
    createBannerAd(): void {
        ...
        if (ConfigHelper.getGameConfig().systemBannerId) {
            ...
            this.bannerAd.onResize((resizeInfo: any) => {
                if (resizeInfo.width !== 360) {
                    this.bannerAd.style.top = windowHeight - resizeInfo.height;
                    this.bannerAd.style.left = (windowWidth - resizeInfo.width) / 2;
                }
            });
        } else {
    ...
    createVideoAd(): void {
        LogUtils.info("createVideoAd ==========", ConfigHelper.getGameConfig().rewardedVideoId);
        if (ConfigHelper.getGameConfig().rewardedVideoId) {
            ...
            this.videoAd = qg.createRewardedVideoAd({
                ...
                success: (result: any) => {
                    LogUtils.info("createVideoAd success=========");
                },
                fail: (error: any, code: any) => {
                    LogUtils.info("createVideoAd fail=========:" + error + code);
                },
                complete: () => {
                    LogUtils.info("createVideoAd complete=========");
                }
            });
            this.videoAd.onLoad(() => {
            ...
            this.videoAd.onError((error: any) => {
                console.log("videoAd error: " + JSON.stringify(error));
                if (this.videoOnError) {
                    this.videoOnError();
                }
            });
            this.videoAd.onClose((closeInfo: any) => {
                if (EngineUtils.isCocos()) {
                    game.resume();
                }
                if (closeInfo && closeInfo.isEnded) {
    ...
    showNativeImageAd(params: any): void {
    ...
    showSysIntertAd(params: any = {}): void {
        ...
        if (AdControlUtils.isShowInter() !== false) {
            ...
            this.inserttitialAd.onLoad((loadInfo: any) => {
                LogUtils.info("inserttitialAd onLoad: ", JSON.stringify(loadInfo));
    ...
    showNativeIntertAd(params: any = {}): void {
    ...
    createNativeAd(): void {
        ...
        if (ConfigHelper.getGameConfig().nativeImageId) {
            ...
            this.nativeImageAd = qg.createNativeAd({
                ...
                fail: (error: any, code: any) => {
                    console.log("createNativeAd fail============", "data: ", error, "code: ", code);
                },
                complete: () => {
                    console.log("createNativeAd complete============");
                }
            });
            this.nativeImageAd.onLoad((loadInfo: any) => {
                LogUtils.info("nativeAd loaded======", JSON.stringify(loadInfo));
                const adItem = loadInfo.adList.pop();
                const adData = {
                    adId: adItem.adId,
                    title: adItem.title,
                    desc: adItem.source,
    ...
    nativeAdReport(adId: string): void {
        if (this.nativeImageAd) {
    ...
    nativeAdClick(adId: string): void {
        if (this.nativeImageAd) {
    ...
    createNativeAdImageUINode(): void {
        ...
        if (adData && params) {
            DefaultNativeTemplate.createNativeAdImageUINode(adData, params, (node: any) => {
                ...
                this.nativeAdReport(adData.adId);
    ...
    createNativeIntertUINode(): void {
        LogUtils.info("createNativeIntertUINode ==========");
        const adData = this.nativeAdIntertData;
        const params = this.nativeAdIntertParams;
        if (adData && params) {
    ...
    login(params: any = {}): void {
        ...
        qg.gameLoginWithReal({
            forceLogin: 1,
            appid: ConfigHelper.getGameInfo().appId,
            success: (loginInfo: any) => {
                LogUtils.log("login success: ", JSON.stringify(loginInfo));
                this.userInfo.openId = loginInfo.playerId;
                this.userInfo.nickName = loginInfo.displayName;
                this.userInfo.avatarUrl = loginInfo.imageUri;
                if (params.resultCallback) {
                    params.resultCallback(true, this.userInfo);
                }
            },
            fail: (error: any, code: any) => {
                LogUtils.error("login error", "data:", error, "code:", code);
                if (params.resultCallback) {
                    params.resultCallback(false);
                }
            }
